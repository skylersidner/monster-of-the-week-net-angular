...
| `--color-surface-hover` | `gray-200` | `slate-700` (`#334155`) | Neutral hover fill (Cancel button, neutral list-item hover) — Yoshi's catalogue addition. Deliberately *not* `slate-800`: that's already `--color-surface-raised`, and the confirm-delete modal's Cancel button sits directly on that surface, so a `slate-800` hover would be invisible there. `slate-700` stays perceptibly lighter than both `surface` (slate-900, ~1.81:1) and `surface-raised` (slate-800, ~1.49:1) — a bigger, more hover-appropriate jump than the raised-surface step itself (~1.22:1), and comfortably legible for any label sitting on it (`text-primary` ~9.4:1, `text-secondary` ~6.6:1). Note: this coincides with `--color-border`'s value — a minor, accepted trade-off (a hairline blending into a hover fill at its own edge), not a functional problem the way reusing `surface-raised` would have been. |
...
| `--color-on-success` | `white` | `slate-900` (`#0f172a`) | Text/icon color on the complete-phase bubble. Same values as `--color-on-accent` — Skyler's call (2026-08-03), formalizing what Luigi's placeholder was already numerically borrowing; see the theming plan's Token Catalogue. |
...
| `--color-badge-mystery` | `amber-100` | `amber-950` (`#451a03`) | See Badges section — this badge already ships (`mystery.adventureType.name` on `mysteries-list.html`/`mystery-detail.html`/`mystery-create-dossier.html`/search results), so the light value is the existing literal, unchanged, not a new pick. |
| `--color-on-badge-mystery` | `amber-700` | `amber-300` (`#fcd34d`) | ~10.4:1 against `--color-badge-mystery` in dark mode; ~4.51:1 against `amber-100` in light mode (existing pairing, hand-verified now — see Accessibility Notes). |
...
**Mystery badge — resolved 2026-08-03, superseding the teal proposal below.** I originally floated this as a reserved slot with no real consumer; that assumption was wrong. `mysteries-list.html`, `mystery-detail.html`, and `mystery-create-dossier.html` already render `mystery.adventureType.name` today via a literal `bg-amber-100 text-amber-700` chip, and `search-results.ts`'s domain-badge map carries the same literal alongside the tokenized Monster/Minion/Bystander/Location badges — so this badge already ships, and already appears *side-by-side* with every other domain badge on the global search results page, which is the exact scenario the hue-avoidance concern below is about. Given that, re-litigating the hue is the wrong call: the hue-preservation philosophy at the top of this section exists precisely to avoid an already-shipped badge "swapping identity," and amber is that already-shipped identity here — replacing it with an unused speculative teal would violate my own stated principle for no functional gain, purely to dodge an adjacency (amber vs. minion's orange) that I can now check directly instead of guessing at. Checked at the values both badges actually use: light `#fde8d8`/`orange-800` (minion) vs. `amber-100`(`#fef3c7`)/`amber-700` (mystery) — a peachy pink-orange chip next to a pale gold one, clearly distinct; dark `orange-950`(`#431407`)/`orange-300`(`#fdba74`) vs. `amber-950`(`#451a03`)/`amber-300`(`#fcd34d`) — orange-300 carries visibly more red, amber-300 reads distinctly more yellow-gold. Close on the hue wheel, not confusable at the values in use. **Decision: keep amber.** Light `amber-100`/`amber-700` (unchanged — the existing literal). Dark `amber-950`(`#451a03`)/`amber-300`(`#fcd34d`) (same deep-950-bg / light-300-text construction as every other badge). Contrast: `amber-300` on `amber-950` ≈ **10.4:1** (hand-computed, comfortably AA); `amber-700` on `amber-100` (the existing, already-shipped light pairing) ≈ **4.51:1** — passes AA for normal text, but tightly; I did not previously verify this since I treated the badge as hypothetical. The old teal proposal (**`teal-100`/`teal-800` light, `teal-950`/`teal-300` dark**) is superseded: it was chosen specifically to dodge every hue already spoken for (red/orange/blue/green/indigo/purple) under the assumption this badge didn't exist yet and its hue was still up for grabs — that assumption no longer holds, so the reasoning that produced it is moot, not wrong at the time.
...
- `on-badge-mystery` (`amber-300`) on `badge-mystery` (`amber-950`): **~10.4:1**
- `on-badge-mystery` (`amber-700`) on `badge-mystery`, light mode (`amber-100`): **~4.51:1** — the existing, already-shipped pairing; passes AA for normal text but with a thin margin, worth a tool spot-check
...
- `surface-hover` (`slate-700`) vs. `surface` (`slate-900`): **~1.81:1**; vs. `surface-raised` (`slate-800`): **~1.49:1** — both are perceptible elevation-style jumps, not text contrast, so no AA threshold applies; `text-primary` on `surface-hover`: **~9.4:1**, `text-secondary` on `surface-hover`: **~6.6:1**
...
4. **Resolved 2026-08-03 — mystery type badge is real, not speculative; kept amber, not teal.** See the Badges section above: the badge already ships (`mysteries-list.html`, `mystery-detail.html`, `mystery-create-dossier.html`, `search-results.ts`) as a literal `bg-amber-100 text-amber-700` chip. Final values: light `amber-100`/`amber-700` (unchanged), dark `amber-950`/`amber-300`. The earlier `teal-100`/`teal-800`/`teal-950`/`teal-300` proposal is superseded.
