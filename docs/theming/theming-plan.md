...
| `--color-toast-success-bg` | Notification toast, success kind (currently bypasses Tailwind via inline `[style.background-color]`) | `#1b6f2a` |
| `--color-toast-error-bg` | Notification toast, error kind (same inline-style bypass) | `#a10808` |
| `--color-badge-mystery-bg` / `-text` | Mystery type badge (if/when mysteries get a type badge) | n/a today — reserved for consistency with the other badges |
| `--color-badge-monster-bg` / `-text` | Monster type badge | `red-100` / `red-700` |
| `--color-badge-minion-bg` / `-text` | Minion type badge | `#fde8d8` / `orange-800` |
| `--color-badge-bystander-bg` / `-text` | Bystander type badge | `blue-100` / `blue-800` |
| `--color-badge-location-bg` / `-text` | Location type badge | `green-100` / `green-900` |
| `--color-badge-archetype-bg` / `-text` | Monster-archetype badge (`monsters-list.html`, `monster-detail.html` — added after both the Tailwind migration and this plan's first draft) | `purple-100` / `purple-700` |
| `--color-weapon-chip-bg` / `-border` / `-text` | Weapon tag chip (list, detail, wizard) | `indigo-50` / `indigo-200` / `indigo-700` |
...
**Update (2026-08-02):** the dark fills for every token above are now defined — see `docs/theming/dark-theme-palette.md`. The two catalogue gaps that document flagged have been folded into the table above as `--color-on-danger` (light `white`, dark `slate-900`, mirroring `--color-on-accent`) and `--color-badge-archetype-bg`/`-text` (light `purple-100`/`purple-700`, unchanged; dark `purple-950`/`purple-300`, per Rosalina's proposed values). The reserved mystery-badge token (`--color-badge-mystery-bg`/`-text`) stays exactly as originally listed — n/a today, no value committed. Rosalina's speculative `teal-100`/`teal-800` (light) / `teal-950`/`teal-300` (dark) proposal for it in her doc is explicitly a placeholder pending a feature that doesn't exist yet; this plan is not adopting it as a real value and no phase below references it.

### Pending decision — awaiting Skyler: token naming vs. Tailwind-generated utility classes

**Not yet applied.** This subsection lays out a problem and a recommendation only — the Token Catalogue above, and every phase's illustrative re-pointing table below, are unchanged until Skyler decides. Nothing here should be read as already-adopted naming.

**The mechanism, confirmed against Tailwind's own `@theme` documentation:** Tailwind v4 generates a utility's name by taking the literal remainder of a `--color-*` custom property's name after the `--color-` prefix and reusing it under *every* color-consuming utility family (`bg-`, `text-`, `border-`, `ring-`, `outline-`, `fill-`, `stroke-`, `decoration-`, `accent-`, `caret-`, `divide-`, and more) — it does not parse or strip any trailing word in the property name as an indicator of "which utility this token is meant for." So `--color-toast-success-bg` does not compile to a utility named `bg-toast-success`; it compiles to `bg-toast-success-bg` (and, if anyone ever wrote it, `text-toast-success-bg`, `border-toast-success-bg`, etc.) — Tailwind literally concatenates the family prefix in front of the *entire* name, "-bg" and all.

**Scope — two distinct effects, not one:**
1. **Real, narrow problem: name stutter for every token whose name already ends in a property word.** This affects exactly the tokens in the catalogue named with a `-bg`/`-text`/`-border` suffix: both toast tokens, all six badge-row tokens (mystery, monster, minion, bystander, location, archetype), and the weapon-chip token. This is a genuine functional risk, not just an aesthetic one: Tailwind v4's on-demand generator raises no build error for a class name that doesn't correspond to a real token — writing `bg-toast-success` (the name this plan's own Phase 1 prose currently implies) silently produces an unstyled element, not a compile failure. That illustrative naming in Phase 1 is flagged there as provisional pending this decision (see Phase 1, Converting the three inline-style spots).
2. **Universal, harmless effect: every token is available under every color-consuming prefix, not just the `-bg`/`-text` ones.** `--color-text-primary` is exactly as capable of producing `bg-text-primary`/`border-text-primary`/`ring-text-primary` as `--color-toast-success-bg` is of producing its own cross-product — this is inherent to how `@theme` works and isn't specific to this catalogue's naming choices. It is not a bundle-size or dead-CSS problem: Tailwind's generator only emits a CSS rule for a class name actually written somewhere Tailwind scans; nothing is pre-generated in bulk, so there's no output bloat from this. The only residual exposure is semantic misuse — a future developer could technically write `bg-text-primary`, using a token designed for a text role as a background fill, and it would "work" with zero warning — worth naming as a soft convention risk, not something this plan needs to engineer around.

**Recommended fix (not applied):** adopt the `on-*` convention the catalogue already uses for `--color-on-accent`/`--color-on-danger`, extended to every fill/text pair currently written with a `-bg`/`-text` suffix:
- `--color-toast-success-bg`/`-text` → `--color-toast-success` (fill) + `--color-on-toast-success` (text-on-fill)
- `--color-toast-error-bg`/`-text` → `--color-toast-error` + `--color-on-toast-error`
- `--color-badge-{domain}-bg`/`-text` → `--color-badge-{domain}` + `--color-on-badge-{domain}`, for all six badge rows
- `--color-weapon-chip-bg`/`-text` → `--color-weapon-chip` + `--color-on-weapon-chip`. The chip's third color (its border) doesn't fit the `on-*` pairing — that convention models "content color on top of a filled surface," not a third independent border role — so it's recommended to rename it to avoid embedding the literal word "border" in the token name at all (e.g. `--color-weapon-chip-line`), so `border-weapon-chip-line` doesn't stutter either.

This resolves the naming collision more cleanly than the current `-bg`/`-text` suffix precisely because it's not a new convention — it's applying the one already established elsewhere in this same catalogue, rather than inventing a second "fill + text" naming shape to sit alongside it.

**Trade-off this accepts:** every doc/template reference across Phases 1–6 that already assumed the illustrative `-bg`/`-text` names (Phase 1's toast bullet, the badge references in Phases 3–5) would need updating to the `on-*` form. This is small in practice — a handful of doc lines, decided *before* any template repointing starts in Phase 1 — precisely because Skyler is being asked now.

**Alternative considered and not recommended:** keep the `-bg`/`-text` suffix shape but drop only `-bg` from the fill token's name (`--color-toast-success` for fill, keep `--color-toast-success-text` for the on-color). Rejected because it's an asymmetric one-off convention that still doesn't match the `--color-on-accent`/`--color-on-danger` precedent already in the catalogue — it would leave two different naming conventions for the same "fill + on-fill-text" shape depending on which token pair you're looking at.
...
**Files changed:** `src/app/layout/page-layout/page-layout.html`, `src/app/pages/settings/*` (new: `settings.html`, `settings.ts`, `settings.spec.ts`), `src/app/app.routes.ts`.
...
- `minion-detail`'s existing `:hover:not(:disabled)` SCSS remnant repoints to token utilities inside its `@apply` block without needing new selectors.
...
**Re-pointing specifics:**
- `monsters-list.html` additionally re-points its monster-archetype badge (`bg-purple-100 text-purple-700`, added after both the original Tailwind migration and this plan's first draft) onto the new `--color-badge-archetype-bg`/`-text` tokens (see Token Catalogue) — same badge pattern as the other four type badges, just added to the codebase later than the rest.
...
- `monsters-list`'s and `minion-detail`'s retained `:hover:not(:disabled)` SCSS remnants repoint cleanly.
...
**Re-pointing specifics:**
- `monster-detail.html` re-points its monster-archetype badge (`bg-purple-100 text-purple-700`) onto `--color-badge-archetype-bg`/`-text`, the same tokens introduced for `monsters-list.html` in Phase 4.
...
- Monster detail's sub-resource grid, weapon-tag chips, archetype badge, and delete-hover guard all repaint correctly.
...
1. **Sweep for stragglers.** Grep the app for any remaining literal palette utility (`bg-`, `text-`, `border-` followed by a Tailwind color name rather than a token name) and any remaining `style=`/`[style]` color binding not already covered by Phases 1–6. Confirm the full-app `grep -rn "style="` / `grep -rln "\[style"` result set from this plan's research phase is now empty for anything color-related (the two `mystery-create-monster-phase.html` `style="width: ..."` bindings and `mystery-create-tracker.html`'s `[style.grid-column]` are structural, not color — they are explicitly **not** in scope for this sweep and should remain as-is).
...
**Inspection Points**
- Full-app grep sweep for literal palette classes and color-bearing inline styles returns nothing unexpected.
...
| `@theme`-declared custom properties don't behave as expected under Angular's Vite/PostCSS pipeline (a new mechanism this app hasn't used yet, versus the existing static `--color-indigo-700`/breakpoint overrides) | Low | High | Verify in Phase 0 with the manual DevTools `.dark` class + no-op check before any template repointing begins in Phase 1. |
