...
**Two authoring/tooling prerequisites**, both verified against the PDF (detail in `custom-moves-ideation.md` 2.5):
1. **`tools/pdf-extract/extract-moves.mjs` needs an inline-list path before authoring.** It segments on bullet glyphs; **6 of the 14 in-scope moves present their options as inline comma- or semicolon-separated runs with no bullets** (Crooked ×2, Changeling, Gumshoe, Professional, Searcher's Guardian) and will silently produce nothing structural for them.
2. **No font-derived `Title` boundary exists anywhere in this content class.** The Move's own name is bold; every option name inside it is regular weight (verified on both an inline and a bulleted case). So every `Title`/`DescriptionText` split here is delimiter-derived — the low-confidence provenance that's an *exception* elsewhere in this schema is the *rule* here, and should be recorded as the default expectation rather than re-flagged per playbook.
